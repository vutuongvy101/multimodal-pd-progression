...
try:
    ...
    from data.loaders_v2.motor_loader import MotorLoader
    from data.loaders_v2.cognitive_loader import CognitiveLoader
    from data.loaders_v2.behavioral_loader import BehavioralLoader
    from data.loaders_v2.sleep_loader import SleepLoader
    from data.loaders_v2.smell_loader import SmellLoader
    ...
    from loaders_v2.motor_loader import MotorLoader
    from data.loaders_v2.cognitive_loader import CognitiveLoader
    from data.loaders_v2.behavioral_loader import BehavioralLoader
    from data.loaders_v2.sleep_loader import SleepLoader
    from data.loaders_v2.smell_loader import SmellLoader
...
class DataIntegrator:
    ...
    def __init__(self, config: Config_v2, normalize_features: bool = True):
        """
        Args:
            config: Configuration object (Config_v2)
        ...
        self.motor_loader = MotorLoader(self.base_dir, config, valid_participants=self.valid_participants)
        self.cognitive_loader = CognitiveLoader(self.base_dir, config, valid_participants=self.valid_participants)
        self.behavioral_loader = BehavioralLoader(self.base_dir, config, valid_participants=self.valid_participants)
        self.sleep_loader = SleepLoader(self.base_dir, config, valid_participants=self.valid_participants)
        self.smell_loader = SmellLoader(self.base_dir, config, valid_participants=self.valid_participants)
    ...
    def load_all_data(self) -> Dict[str, pd.DataFrame]:
        ...
        motor_df = self.motor_loader.load()
        
        try:
            cognitive_df = self.cognitive_loader.load()
        except:
            print("  ⚠️  Cognitive assessments not available, continuing without them")
            cognitive_df = pd.DataFrame(columns=['PATNO', 'EVENT_ID'])
        
        try:
            behavioral_df = self.behavioral_loader.load()
        except:
            print("  ⚠️  Behavioral assessments not available, continuing without them")
            behavioral_df = pd.DataFrame(columns=['PATNO', 'EVENT_ID'])
        
        try:
            sleep_df = self.sleep_loader.load()
        except:
            print("  ⚠️  Sleep assessments not available, continuing without them")
            sleep_df = pd.DataFrame(columns=['PATNO', 'EVENT_ID'])
        
        try:
            smell_df = self.smell_loader.load()
        except:
            print("  ⚠️  Smell assessments not available, continuing without them")
            smell_df = pd.DataFrame(columns=['PATNO', 'EVENT_ID'])
        ...
        longitudinal_df = motor_df.copy()
        longitudinal_df = self._merge_longitudinal_data(longitudinal_df, cognitive_df, '_cognitive')
        longitudinal_df = self._merge_longitudinal_data(longitudinal_df, behavioral_df, '_behavioral')
        longitudinal_df = self._merge_longitudinal_data(longitudinal_df, sleep_df, '_sleep')
        longitudinal_df = self._merge_longitudinal_data(longitudinal_df, smell_df, '_smell')
    ...
    def prepare_final_dataset(self) -> Dict:
        ...
        data['metadata'] = {
            ...
            'overall_motor_severity_score': [c for c in data['longitudinal'].columns if c in self.config.features.overall_motor_severity_score]
        }
        ...
        print(f"  Overall motor severity score available: {', '.join(data['metadata']['overall_motor_severity_score'])}")
    ...
    def create_feature_vectors_with_visit_index(self, prepared_data: Dict = None, min_visits: int = 2) -> Dict:
        ...
        if prepared_data is None:
            ...
            try:
                ...
                print(f"  ⚠️ Cognitive data not available: {e}")
                cognitive_df = pd.DataFrame(columns=['PATNO', 'EVENT_ID'])
            
            try:
                behavioral_df = self.behavioral_loader.load()
            except Exception as e:
                print(f"  ⚠️ Behavioral data not available: {e}")
                behavioral_df = pd.DataFrame(columns=['PATNO', 'EVENT_ID'])
            
            try:
                sleep_df = self.sleep_loader.load()
            except Exception as e:
                print(f"  ⚠️ Sleep data not available: {e}")
                sleep_df = pd.DataFrame(columns=['PATNO', 'EVENT_ID'])
            
            try:
                smell_df = self.smell_loader.load()
            except Exception as e:
                print(f"  ⚠️ Smell data not available: {e}")
                smell_df = pd.DataFrame(columns=['PATNO', 'EVENT_ID'])
            ...
            motor_df = prepared_data.get('longitudinal', pd.DataFrame())
            cognitive_df = prepared_data.get('cognitive', pd.DataFrame(columns=['PATNO', 'EVENT_ID']))
            behavioral_df = prepared_data.get('behavioral', pd.DataFrame(columns=['PATNO', 'EVENT_ID']))
            sleep_df = prepared_data.get('sleep', pd.DataFrame(columns=['PATNO', 'EVENT_ID']))
            smell_df = prepared_data.get('smell', pd.DataFrame(columns=['PATNO', 'EVENT_ID']))
        ...
        visit_index = visit_index_builder.build_from_sources(
            source_dfs=[motor_df, cognitive_df, behavioral_df, sleep_df, smell_df, medication_df, age_at_visit_df],
        ...
        behavioral_cols = [c for c in self.config.features.behavioral_features 
                        if c in behavioral_df.columns]
        sleep_cols = [c for c in self.config.features.sleep_features 
        ...
        for patno in valid_patnos:
            ...
            for _, visit_row in patient_visits.iterrows():
                ...
                # Motor features from motor assessments
                ...
                if len(cognitive_visit) > 0 and cognitive_cols:
                    cognitive_values, cognitive_mask = self.create_missingness_masks(cognitive_visit, cognitive_cols, scaler=self.non_motor_scaler)
                    visit_dict['cognitive_values'] = cognitive_values[0]
                    visit_dict['cognitive_mask'] = cognitive_mask[0]
                else:
                    visit_dict['cognitive_values'] = np.zeros(len(cognitive_cols) if cognitive_cols else 0, dtype=np.float32)
                    visit_dict['cognitive_mask'] = np.ones(len(cognitive_cols) if cognitive_cols else 0, dtype=np.float32)
                
                # Behavioral features
                behavioral_visit = behavioral_df[(behavioral_df['PATNO'] == patno) & (behavioral_df['EVENT_ID'] == event_id)] if len(behavioral_df) > 0 else pd.DataFrame()
                if len(behavioral_visit) > 0 and behavioral_cols:
                    behavioral_values, behavioral_mask = self.create_missingness_masks(behavioral_visit, behavioral_cols, scaler=self.non_motor_scaler)
                    visit_dict['behavioral_values'] = behavioral_values[0]
                    visit_dict['behavioral_mask'] = behavioral_mask[0]
                else:
                    visit_dict['behavioral_values'] = np.zeros(len(behavioral_cols) if behavioral_cols else 0, dtype=np.float32)
                    visit_dict['behavioral_mask'] = np.ones(len(behavioral_cols) if behavioral_cols else 0, dtype=np.float32)
                
                # Sleep features
                sleep_visit = sleep_df[(sleep_df['PATNO'] == patno) & (sleep_df['EVENT_ID'] == event_id)] if len(sleep_df) > 0 else pd.DataFrame()
                if len(sleep_visit) > 0 and sleep_cols:
                    sleep_values, sleep_mask = self.create_missingness_masks(sleep_visit, sleep_cols, scaler=self.non_motor_scaler)
                    visit_dict['sleep_values'] = sleep_values[0]
                    visit_dict['sleep_mask'] = sleep_mask[0]
                else:
                    visit_dict['sleep_values'] = np.zeros(len(sleep_cols) if sleep_cols else 0, dtype=np.float32)
                    visit_dict['sleep_mask'] = np.ones(len(sleep_cols) if sleep_cols else 0, dtype=np.float32)
                
                # Smell features
                smell_visit = smell_df[(smell_df['PATNO'] == patno) & (smell_df['EVENT_ID'] == event_id)] if len(smell_df) > 0 else pd.DataFrame()
                if len(smell_visit) > 0 and smell_cols:
                    smell_values, smell_mask = self.create_missingness_masks(smell_visit, smell_cols, scaler=self.non_motor_scaler)
                    visit_dict['smell_values'] = smell_values[0]
                    visit_dict['smell_mask'] = smell_mask[0]
                else:
                    visit_dict['smell_values'] = np.zeros(len(smell_cols) if smell_cols else 0, dtype=np.float32)
                    visit_dict['smell_mask'] = np.ones(len(smell_cols) if smell_cols else 0, dtype=np.float32)
        ...
        if not motor_df.empty:
            slopes_df = self.compute_progression_slopes(motor_df)
        else:
